...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    int num_hWalls = horiz*(vert-1);
    int num_vWalls = (horiz-1)*vert;
    int w = num_hWalls + num_vWalls;
    int[] arrayOfWall = new int[w];
    for (i=0; i<arrayOfWall.length; i++) {
      arrayOfWall[i] = i;
    }
    System.out.printf("horizontalSize = %d, verticalSize = %d, num_hWalls = %d, num_vWalls = %d, arrayOfWall.length = %d\n", horiz, vert, num_hWalls, num_vWalls, arrayOfWall.length);

    for (i=w; i>0; i--) {
      int rand_num = randInt(i);
      int temp = arrayOfWall[rand_num];
      arrayOfWall[rand_num] = arrayOfWall[i-1];
      arrayOfWall[i-1] = temp;
    }

    for (i=0; i<arrayOfWall.length; i++) {
      int cella_x, cella_y, cellb_x, cellb_y, vnum,wall_type;
      if (arrayOfWall[i] < num_hWalls) {
        cella_x = arrayOfWall[i] % horiz;
        cellb_x = arrayOfWall[i] % horiz;
        cella_y = arrayOfWall[i] / horiz;
        cellb_y = (arrayOfWall[i] / horiz) + 1;
        wall_type = 0;
      } else {
        vnum = arrayOfWall[i] - num_hWalls;
        cella_x = vnum / vert;
        cellb_x = (vnum / vert) + 1;
        cella_y = vnum % vert;
        cellb_y = vnum % vert;
      ...
      int cella_num, cellb_num;
      cella_num = cella_x + cella_y*horiz;
      cellb_num = cellb_x + cellb_y*horiz;
      if (sets.find(cella_num) != sets.find(cellb_num)) {
        if (wall_type == 0) {
        ...
        sets.union(sets.find(cella_num), sets.find(cellb_num));
      }
    }
    System.out.println("After all unions, the result of disjoint Sets:");
   // sets.check();
    /**
